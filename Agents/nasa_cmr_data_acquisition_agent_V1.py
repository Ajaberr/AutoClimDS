...
class CredentialManager:
    ...
    def get_credential(self, service: str, credential_type: str, prompt_message: str = None) -> Optional[str]:
        """
        Get a credential without blocking terminal input.
        Checks environment variables, Streamlit secrets (if available), or returns None.
        ...
        # Try Streamlit Session State (if running in Streamlit)
        try:
            import streamlit as st
            # Check session state for manually entered keys (e.g. from sidebar)
            st_key = f"auth_{service}_{credential_type}"
            if hasattr(st, 'session_state') and st_key in st.session_state:
        ...
        # If missing, do NOT prompt with input() as it halts the server.
        # Instead, log a warning and return None. The Agent can then decide to fail or ask via UI.
        
        # Only log prompt once per credential to avoid spam
        if key not in self._prompted:
            self._prompted.add(key)
            print(f"⚠️  MISSING CREDENTIAL: {service} {credential_type}")
            print(f"   Please set env var: {env_var_name} or configure in UI.")
...
def create_nasa_cmr_agent():
    ...
    class AskDataProcessingFollowUpTool(BaseTool):
        ...
        def _run(self, question_context: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
            ...
            # Non-blocking return for Streamlit
            output += f"⚠️ **Interactive Mode Info:**\n"
            output += f"In this web interface, I cannot wait for your input mid-execution. \n"
            output += f"Please provide these details in your next query or use the specific tools listed above."
    ...
    ⚠️ DATA ANALYSIS WARNING - CRITICAL:
    When writing code to aggregate data from multiple weather stations (e.g., NOAA GHCND data):
    - NEVER use SUM() to combine data across different stations for the same timestamp.
    - ALWAYS use MEAN() or AVERAGE() to represent the region.
    - REASON: Summing 90 stations' rainfall will result in 90x the actual rainfall, creating physically impossible values (e.g., 700mm/day).
    - Exception: Only use SUM() if you are aggregating over TIME (e.g., total monthly rainfall for a single station).
