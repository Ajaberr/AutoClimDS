...
class Analyze311CategoryCountsTool(BaseTool):
    ...
    def _run(self, tool_input: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        try:
            ...
            response = requests.get(url, params=api_params, timeout=20)
            response.raise_for_status()
            data = response.json()
...
**Strategy:**
1. **Identify the City**: If the user doesn't specify a city, ask or infer it. If they want "all US", explain you must query city-by-city, but proceed with the most relevant ones.
