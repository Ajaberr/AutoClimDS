...
NYC_311_V3_URL = "https://data.cityofnewyork.us/resource/erm2-nwe9.json"
...
class Analyze311CategoryCountsTool(BaseTool):
    ...
    def _run(self, tool_input: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        try:
            ...
            try:
                response = requests.get(url, params=api_params, headers=get_socrata_headers(), timeout=60)
                response.raise_for_status()
                data = response.json()
            except requests.exceptions.Timeout:
                # Retry with smaller limit or simplified query if timeout occurs
                api_params["$limit"] = 20
                response = requests.get(url, params=api_params, headers=get_socrata_headers(), timeout=60)
                response.raise_for_status()
                data = response.json()
            ...
            return f"Error analyzing counts: {e}. Socrata API query timed out; try narrowing the date range or using query_nyc311_v3 with a smaller limit."
...
class QueryNYC311V3Tool(BaseTool):
    ...
    def _run(self, tool_input: str, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        headers = {}
        # Only send token header if it's set and not a placeholder
        if SOCRATA_APP_TOKEN and not SOCRATA_APP_TOKEN.startswith("your_"):
            headers["X-App-Token"] = SOCRATA_APP_TOKEN

        try:
            resp = requests.get(NYC_311_V3_URL, params={"$query": soql}, headers=headers, timeout=30)
            if resp.status_code != 200:
                # Fallback to individual parameter style if $query fails
                v2_params = {"$limit": params.get("limit", 1000), "$offset": params.get("offset", 0)}
                if "where" in params:
                    v2_params["$where"] = params["where"]
                if "select" in params:
                    v2_params["$select"] = params["select"]
                if "order" in params:
                    v2_params["$order"] = params["order"]
                resp = requests.get(NYC_311_V3_URL, params=v2_params, headers=headers, timeout=30)
            resp.raise_for_status()
            data = resp.json()
            if isinstance(data, list):
                records = data
            else:
                records = self._parse_v3_response(data)
        except Exception as e:
            return f"NYC 311 API error: {e}"
...
1. **For NYC queries**: Always use `query_nyc311_v3` first — it is faster and more reliable.
   - For multi-month or annual queries, query in 1-month date intervals or aggregate by month to prevent backend delays.
