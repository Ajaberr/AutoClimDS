...
def create_climate_research_orchestrator(session_id=None, **kwargs):
...
⚡ MONTHLY CHUNKING & EFFICIENT DATA ACQUISITION RULE (CRITICAL):
- When requesting annual or multi-month data (e.g. full year 2023):
  1. For 311 Service Requests: Query by 1-month date intervals or aggregate by month to avoid slow multi-million row table scans.
  2. For NOAA Weather Data: Request monthly summaries (GSOM) or 1-month date windows (`startdate='2023-01-01'`, `enddate='2023-01-31'`).
  3. For ERA5 / Copernicus: Specify month lists (`month=['01','02',...]`) in single batch API calls instead of day-by-day iterations.
